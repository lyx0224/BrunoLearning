...
class RoundedLoadingButtonState extends State<RoundedLoadingButton>
  ...
  void initState() {
    ...
    // 添加到controller的stream中，方便外部监听
    _state.stream.listen((event) {
      if (!mounted) return;
      widget.controller._controllerState.sink.add(event);
    });
...
class RoundedLoadingButtonController {
  ...
  final BehaviorSubject<ButtonState> _controllerState =
      BehaviorSubject<ButtonState>.seeded(ButtonState.idle);

  /// A read-only stream of the button state
  Stream<ButtonState> get stateStream => _controllerState.stream;

  /// Gets the current state
  ButtonState? get currentState => _controllerState.value;
